...
import mx.gob.sedena.ui.ManageAccountActivity;
import mx.gob.sedena.ui.CargandorLocalizacion;
import mx.gob.sedena.ui.WelcomeActivity;
import mx.gob.sedena.utils.ProveedorLocalización;
...
public class ConversationActivity extends XmppActivity
	...
	public static final int ATTACHMENT_CHOICE_CONTACT = 0x0306;
	...
	protected String obtenerContacto() {
		Conversation conversation = getSelectedConversation();

		if (conversation != null) {
			return conversation.obtenerContacto().obtenerJid();
		} else {
			return "";
		}
	}
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		XmppActivity.PASSFRASE = this.PASSFRASE;
        XmppConnectionService.PASSFRASE = this.PASSFRASE;
	...
	private void openConversation() {
		selectEncryptionDialog2(getSelectedConversation());
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (isConversationsOverviewVisable() && isConversationsOverviewHideable()) {
			...
			if (this.getSelectedConversation() != null) {
				if (this.getSelectedConversation().getNextEncryption() != Message.ENCRYPTION_NONE ) { //added true
		...
		if (true) { //added Config.supportOmemo()
			...
			public void onPresenceSelected() {
				...
				switch (attachmentChoice) {
					...
					case ATTACHMENT_CHOICE_RECORD_VOICE:
						intent = new Intent(ConversationActivity.this, GrabadoraDeAudio.class);
						Toast.makeText(ConversationActivity.this,"Grabando Audio", Toast.LENGTH_LONG).show();
						break;
					case ATTACHMENT_CHOICE_LOCATION:
						...
						if ( !manager.isProviderEnabled( LocationManager.GPS_PROVIDER ) ) {
							...
							Toast.makeText(ConversationActivity.this, "Enciende Tu GPS para mayor Precision", Toast.LENGTH_LONG).show();
							//buildAlertMessageNoGps();
						}else {
							Toast.makeText(ConversationActivity.this, "Buscando su Ubicación", Toast.LENGTH_LONG).show();
							Intent i = new Intent(getApplicationContext(), CargandorLocalizacion.class);
							startActivity(i);
							ProveedorLocalización.solicitarActualizacion(ConversationActivity.this,
									new ProveedorLocalización.CambioDeLocalizacion() {
										@Override
										public void onNewLocationAvailable(ProveedorLocalización.CoordenadasGPS location) {
											if (location.latitude != -1 && location.latitude != -1) {
												Intent i = new Intent(getApplicationContext(), ConversationActivity.class);
												startActivity(i);
											}
											mConversationFragment.setmEditMessage("https://maps.google.com/maps?q=" + location.latitude + "," + location.longitude);
										}
									});
						}
						break;
					case ATTACHMENT_CHOICE_CONTACT:
						Toast.makeText(ConversationActivity.this,"Enviando Contacto", Toast.LENGTH_LONG).show();
						Intent shareIntent = new Intent();
						shareIntent.setAction(Intent.ACTION_SEND);
						shareIntent.putExtra(Intent.EXTRA_TEXT,"xmpp:"+obtenerContacto());
			...
			public boolean onMenuItemClick(MenuItem item) {
				switch (item.getItemId()) {
					...
					case R.id.attach_location:
						...
						break;
					case R.id.attach_choose_contact:
		...
		if (fragment != null) {
			...
			*/
			...
			//if (!conversation.getAccount().getAxolotlService().isConversationAxolotlCapable(conversation)) {
				axolotl.setEnabled(true); //modificado era falso
			//}
	...
	protected void selectEncryptionDialog2(final Conversation conversation) {
	...
	private void buildAlertMessageNoGps() {
		...
		builder.setMessage("Al parecer tu GPS no esta habilitado, Desea habilitarlo?")
				.setCancelable(false)
